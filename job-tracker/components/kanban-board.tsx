...
function DroppableColumn({
  ...
  return (
    <Card className="min-w-[300px] flex-shrink-0 shadow-md p-0">
      ...
      <CardContent
        ref={setNodeRef} // HERE WE REFER TO THE DROPPABLE ITEM WE WANT
        className={`
          space-y-2 pt-4 bg-gray-50/50 min-h-[400px] rounded-b-lg
...
function SortableJobCard({
  ...
  return (
    <div ref={setNodeRef} style={onDraggingStyle}>
      <JobApplicationCard
        job={job}
        columns={columns}
...
export default function KanbanBoard({ board, userId }: KanbanBoardProps) {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    if (targetColumn) {
      ...
      const targetIndexInFiltered = allJobsInTargetFiltred.findIndex(j => j._id === overId)
      ...
      if(targetIndexInFiltered !== -1) {
        ...
        newOrder = allJobsInTargetFiltred.length
      }
    } else {
      return
    }
